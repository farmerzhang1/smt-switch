...
curl http://ftp.gnu.org/gnu/bison/bison-3.8.tar.xz --output $DEPS/bison-3.8.tar.xz

if [ ! -f "$DEPS/bison-3.8.tar.xz" ]; then
    echo "It seems like downloading bison to $DEPS/bison-3.8.tar.xz failed"
    exit 1
fi
